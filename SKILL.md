...
4. Run `python3 scripts/check_fit.py <output_dir>/<slug>` to catch any page whose content is cut off (exit `3` + one line per offender; exit `0` when clean). It reports two kinds: `page NN OVERFLOW by …px` — content runs past the slide's bottom edge; and `page NN CLIPPED — <table> by …px` — a box inside the page swallows its own rows while the page itself measures as fitting (marp gives `<table>` `overflow:auto`, and a slide is never scrolled, so those rows are simply unreadable). For each offender, **thin the page** (split it, cut words, or move a display formula to its own page — never shrink the font) and re-run `compile_marp.sh` + `check_fit.py` until it exits `0`. A CLIPPED page needs the *box* itself to fit: check by eye that the last row is back, because trimming a table's cell text can reflow the column widths and buy nothing.
...
- `scripts/check_fit.py` — loads `slides.html` in headless Chrome and reports any page whose content is cut off: OVERFLOW (past the slide box) and CLIPPED (a box inside the page cutting its own rows, which a slide-level measurement cannot see). Exit `3` on either. Run after `compile_marp.sh`; no network needed.
